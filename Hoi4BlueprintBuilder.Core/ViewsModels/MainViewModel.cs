...
)
{
    ...
    private async Task CreateNewFocusTreeFile()
    {
        ...
        {
            await messageBoxService.ShowAsync("文件已存在, 无法创建同名文件.", "创建失败", MessageBoxIcon.Error);
            return;
        }
        ...
        try
        {
            ...
            userStatusService.CurrentSelectedFile = new SystemFileItem(filePath, true, null);
            tabViewService.AddTabFromIoc<EditorCanvasView>(filePath);
        }
        catch (Exception e)
        {
            Log.Error(e, "创建国策树文件失败");
            await messageBoxService.ShowAsync($"创建国策树文件失败: {e.Message}", "创建失败", MessageBoxIcon.Error);
        }
    }
